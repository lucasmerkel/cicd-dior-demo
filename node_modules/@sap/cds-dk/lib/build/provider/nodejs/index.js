...
class NodejsModuleBuilder extends BuildTaskHandlerEdmx {
    ...
    // REVISIT: fix for simple mono repos using npm workspaces
    // Support for HANA native artifacts defined in sub-modules required
    async _addWorkspaceDependencies(dest) {
        try {
            const workspaceRoot = path.resolve(cds.root, this.getBuildOption("workspaceRoot") ?? '.')
            if (workspaceRoot !== cds.root && !fs.existsSync(workspaceRoot)) {
                throw new BuildError(`The given workspaceRoot path ${workspaceRoot} does not exist`)
            }
            const packageJson = require(path.join(dest, 'package.json'))
            const workspaces = workspaceRoot === cds.root ? packageJson.workspaces : require(path.join(workspaceRoot, 'package.json')).workspaces
            if (workspaces?.length) {
                this.logger._debug && this.logger.debug(`Existing npm workspace configuration '${JSON.stringify(workspaces, ' ')}'`)
                const { tar } = require('../../../../lib').utils
                const { dependencies } = packageJson
                const folders = fs.readdirSync(workspaceRoot).filter(res => {
                    try {
                        return fs.lstatSync(path.join(workspaceRoot, res)).isDirectory()
                    } catch (e) {
                        return false
                    }
                })
                this.logger._debug && this.logger.debug(`Existing workspace subfolders '${JSON.stringify(folders, ' ')}'`)

                const workspaceNames = new Set()
                let changed
                for (const workspace of workspaces) {
                    //REVISIT: workspaces must only use simple glob like 'foo*', nested folders are not supported
                    const regex = new RegExp('^' + workspace.split('/').pop().replace(/\*/g, '.*') + '$')
                    for (const folder of folders) {
                        if (!workspaceNames.has(folder) && folder.match(regex)) {
                            workspaceNames.add(folder)
                            ...
                            if (dependencies[plugin.name] === '*') {
                                const tarFilePath = path.join(dest, tarFile)
                                dependencies[plugin.name] = `file:${tarFile}`
                                await tar.czfd(tarFilePath, path.join(workspaceRoot, folder))
                                this.pushFile(tarFilePath)
                                changed = true
                            }
                        }
                    }
                ...
                if (changed) {
                    return this.write(JSON.stringify(packageJson, null, '  ')).to(path.join(dest, 'package.json'))
                }
            }
        } catch (e) {
            this.pushMessage(`Failed to add npm workspace dependencies: ${e.message}`)
            this.logger.debug(e)
        }
    }
